...
// nameDe/nameSq carried over from the app's previous hardcoded
// category_translations.dart map — already-reviewed, in-use
// translations, not new ones invented for this migration.
const CATEGORIES = [
  { name: 'Restaurants', nameDe: 'Restaurants', nameSq: 'Restorante', icon: 'restaurant_outlined', sort: 1 },
  { name: 'Cafes', nameDe: 'Cafés', nameSq: 'Kafene', icon: 'coffee_outlined', sort: 2 },
  { name: 'Shops', nameDe: 'Geschäfte', nameSq: 'Dyqane', icon: 'storefront_outlined', sort: 3 },
  { name: 'Services', nameDe: 'Dienstleistungen', nameSq: 'Shërbime', icon: 'build_outlined', sort: 4 },
  { name: 'Health', nameDe: 'Gesundheit', nameSq: 'Shëndeti', icon: 'fitness_center_outlined', sort: 5 },
  { name: 'Entertainment', nameDe: 'Unterhaltung', nameSq: 'Argëtim', icon: 'local_movies_outlined', sort: 6 },
  { name: 'Other', nameDe: 'Sonstiges', nameSq: 'Tjetër', icon: 'category_outlined', sort: 7 },
];

async function seedCategories() {
  for (const cat of CATEGORIES) {
    await pool.query(
      `INSERT INTO categories (name, name_de, name_sq, icon_name, sort_order)
       VALUES (?, ?, ?, ?, ?)
       ON DUPLICATE KEY UPDATE
         name_de = VALUES(name_de), name_sq = VALUES(name_sq),
         icon_name = VALUES(icon_name), sort_order = VALUES(sort_order)`,
      [cat.name, cat.nameDe, cat.nameSq, cat.icon, cat.sort],
    );
  }
...
// schema.sql) — the exact English copy that used to be hardcoded
// independently in the mobile app's localization files and the
// website's next-intl messages/literal JSX, so nothing goes blank on
// either client the first time this runs. social_links.facebook/
// instagram/twitter carry over the same placeholder handles ("albmap")
// the mobile app's About Us screen used to hard-code — almost certainly
// not real accounts; an admin should confirm/replace them via the admin
// portal's Content page.
const ABOUT_US_EN = {
...
// about_us/privacy_policy/terms_conditions now require all 3 languages
// (see content.service.js's LOCALIZED_KEYS/SUPPORTED_LOCALES) — the
// German/Albanian slots below are seeded as a duplicate of the English
// copy, NOT a real translation, purely so a fresh install/migration has
// *something* in every language instead of a validation error or a
// blank page. This is a starting point, not finished copy — especially
// for the legal pages, an admin should replace it with a proper
// professional translation via the admin portal's Content page before
// relying on it. social_links is unchanged — URLs aren't localized.
const SITE_CONTENT = {
  about_us: { en: ABOUT_US_EN, de: ABOUT_US_EN, sq: ABOUT_US_EN },
  social_links: {
  ...
  privacy_policy: { en: PRIVACY_POLICY_EN, de: PRIVACY_POLICY_EN, sq: PRIVACY_POLICY_EN },
  terms_conditions: { en: TERMS_CONDITIONS_EN, de: TERMS_CONDITIONS_EN, sq: TERMS_CONDITIONS_EN },
};
